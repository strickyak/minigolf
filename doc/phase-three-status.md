# Phase Three Status Report: Optimizations, Inlining & Telemetry
...
- **Whole-Program Function Inlining (`opt/inline.go`)**: Inlining of tiny leaf functions (`peekb`, `pokeb`, `peekw`, `pokew`, `vpeekb`, `vpeekw`) and single-callsite functions, while strictly respecting safety boundaries (skipping `defer`, destructors, `setjmp`/`longjmp`, and escaped local addresses).
- **Branch Layout & Inversion (`m6809/backend.go` & `m6809/peephole.go`)**: Inverting branch conditions to fall through directly to the sequentially next basic block; eliminating jumps over jumps and redundant unconditional branches.
- **Fused Compare + Branch (`m6809/backend.go`)**: Fusing single-use comparison operations directly into conditional branch instructions (`cmpd`/`cmpb`/`tstb` followed directly by conditional branch, bypassing boolean materialization and stack stores).
...
## 2. Telemetry & Performance Results

Telemetry comparison between **Phase Two** (`perf-2026-09-14-130828-phase-two-complete`) and **Phase Three** (`perf-2026-09-14-190049-phase-three-complete`):

| Metric | Phase Two Baseline | Phase Three Final | Absolute Delta | Percentage Delta |
|---|---|---|---|---|
| **Default Code Size (bytes)** | 443,493 | 359,687 | -83,806 bytes | **-18.90%** |
| **Default Execution Cycles** | 48,653,411 | 30,752,725 | -17,900,686 cycles | **-36.79%** |
| **All-Variants Code Size (bytes)** | 2,710,645 | 2,227,206 | -483,439 bytes | **-17.83%** |
| **All-Variants Execution Cycles** | 224,426,372 | 119,893,166 | -104,533,206 cycles | **-46.58%** |
