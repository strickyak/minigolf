...
"$ASM68K" -o _tmp/k.srec -l _tmp/k.list "$SCRIPT_DIR/m68k/cstart.asm" _tmp/main.s >&2
...
if test -z "$TRACE"
then
    "$HATVAN_VMK" --print-cycles _tmp/k.srec
else
    "$HATVAN_VMK" --trace --print-cycles _tmp/k.srec
fi
