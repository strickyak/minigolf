...
func (b *Builder) buildFunc(s *ast.FuncStatement) {
	...
	//		if false { // NOT NEEDED, was a defense-in-depth.
	...
	if idxExpr, ok := e.Function.(*ast.IndexExpression); ok {
		...
		if rawFuncName != "" {
			...
			if _, ok := b.funcs[funcName]; !ok {
				if tmpl, ok := b.tm.genericTemplates[rawFuncName]; ok {
	...
	if sel, ok := e.Function.(*ast.SelectorExpression); ok {
		...
		if _, exists := b.funcs[funcName]; !exists {
			if instInfo, ok := b.tm.instantiatedTypes[baseType]; ok {
				...
				if tmpl, ok := b.tm.genericTemplates[rawGenericFuncName]; ok {
...
func (b *Builder) eval(expr ast.Expression) ExprResult {
	...
	case *ast.SelectorExpression:
		...
		st, ok := b.tm.typeDefsAST[structName]
		if !ok {
	...
	case *ast.CompositeLit:
		...
		st, ok := b.tm.typeDefsAST[typ.Name]
		if !ok {
	...
	case *ast.PrefixExpression:
		if e.Operator == "^" {
			right := b.buildExpr(e.Right)
			typ := right.Type()
			// Bitwise complement: x ^ all-ones  (matches Go's unary ^ semantics).
			var allOnes Value
			if typ.Equals(TypeByte) {
				allOnes = b.addInstr(&ConstByte{BaseInstruction: BaseInstruction{Typ: TypeByte}, Val: 0xFF}, nil)
			} else {
				allOnes = b.addInstr(&ConstWord{BaseInstruction: BaseInstruction{Typ: typ}, Val: 0xFFFF}, nil)
			}
			val := b.addInstr(&BinaryOp{BaseInstruction: BaseInstruction{Typ: typ}, Op: "xor", Left: right, Right: allOnes}, expr)
			return ExprResult{IsLValue: false, Value: val, Typ: typ}
		}
		if e.Operator == "!" {
...
func (b *Builder) buildAddressOf(expr ast.Expression) (Value, Type) {
	if idxExpr, ok := expr.(*ast.IndexExpression); ok {
		...
		if base.Typ.IsASlice() {
			...
			eltType := TypeByte
			instInfo, ok := b.tm.instantiatedTypes[baseType]
...
func (b *Builder) tryResolve(item *GlobalItem) (err error) {
	...
	case ItemGenericType:
		...
		b.tm.genericTemplates[item.QName] = &GenericTemplate{
	...
	case ItemType:
		...
		if st, ok := s.BaseType.(*ast.StructType); ok {
			b.tm.typeDefsAST[item.QName] = st
		}
...
func (b *Builder) evalConstantExpr(expr ast.Expression, targetTyp Type) Value {
	...
	case *ast.PrefixExpression:
		if e.Operator == "-" {
			...
			if cb, ok := val.(*ConstByte); ok {
				return &ConstByte{BaseInstruction: BaseInstruction{Typ: targetTyp}, Val: cb.Val ^ 0xFF}
			}
		}
	case *ast.Identifier:
		fullName := e.FullName()
	...
	case *ast.CompositeLit:
		...
		st, ok := b.tm.typeDefsAST[targetTyp.Name]
		if !ok {
